! the subroutines here are developed to read the pressure and velocity data from the main solver
! copyright (c) Anil A. Aksu, 2018

subroutine pread(pfiles,M)
!the subroutine is developed to read the pressure
integer i
! the size of the pressure time series
integer,intent(in)::M
...
! the file names are stored in depot.dat file
open(15,file='depot.dat',status='unknown')

do i=1,M
	! the pressure files for 1-9
	if (i<10) then 
		write(15,210) 'Press_',i,'.dat'
		210 FORMAT(A12,I1,A4)
		goto 300
	end if 
	! the pressure files for 10-99
	if (i<100) then 
		write(15,220) 'Press_',i,'.dat'
		220 FORMAT(A12,I2,A4)
		goto 300
	end if 
	! the pressure files for 100-999
	if (i<1000) then 
...
! depot.dat file is generated now it is read to get the name of pressure data
close(15)

open(16,file='depot.dat',status='old')
do i=1,M
	! the name of pressure files are stored in pfiles array
	read(16,*) pfiles(i)
end do
